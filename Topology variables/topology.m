...
n_excel = 10;
num = 0;
images = xlsread(['integrated_images/', num2str(num), '.xlsx'], 'images');
images = reshape(images, [10, 50, 50]);
soil_parameters = xlsread(['integrated_images/', num2str(num), '.xlsx'], 'soil_parameters');
...
try
    ...
    dlmwrite(['processing/', num2str(start_num+num*n_excel), 'to', num2str(end_num+num*n_excel), 'processing.txt'], i+num*n_excel);
    mphsave(model, ['dispersion_curves/', num2str(i+num*n_excel)]);
catch
    dlmwrite(['errors/', num2str(i+num*n_excel), '.txt'], i+num*n_excel);
end
toc
end
