...
def MLP(p):
    ...
    l1 = tf.layers.batch_normalization(l1)   # batch_normalization有利于训练收敛和避免过拟合
    l1 = tf.nn.relu(l1)
...
def training_model(num, iterations):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(iterations):
            ...
            if (epoch+1)%100 == 0:
                saver.save(sess, 'models/MLP_for_P.ckpt')
                print('MLP has been saved!')

                # 记录学习历史
                book = xlsxwriter.Workbook('results/learning_curves_for_P_by_MLP.xlsx')
...
def predictor(num):
    ...
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        saver.restore(sess, 'models/MLP_for_P.ckpt')
    ...
    book = xlsxwriter.Workbook('results/prediction_accuracy_for_test_P_by_MLP.xlsx')
